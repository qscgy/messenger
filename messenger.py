...
class MessengerClient:
    ...
    def encrypt(self, mk, plaintext, ad):
        aesgcm = AESGCM(mk)
        nonce = os.urandom(16)
        ct = aesgcm.encrypt(os.urandom(16), plaintext, ad)
        return ct, nonce

    def ratchet_encrypt(self, state, plaintext):
        state.CKs, mk = self.kdf_ck(state.CKs)
        header = Certificate(self.name, state.DHs[1])
        ct, nonce = self.encrypt(mk, plaintext, header.to_bytes())
        return ct, nonce, header

    def decrypt(self, mk, ct, ad, nonce):
        aesgcm = AESGCM(mk)
        message = aesgcm.decrypt(nonce, ct, ad)
        return message.decode('ascii')
    
    def dh_ratchet(self, state: ConnState, header):
    ...
    def ratchet_decrypt(self, state, header, cipertext):
        if header[0].pkey != state.DHr:
            self.dh_ratchet(state, header[0])
        state.CKr, mk = self.kdf_ck(state.CKr)
        return self.decrypt(mk, cipertext, header[0].to_bytes(), header[1])
